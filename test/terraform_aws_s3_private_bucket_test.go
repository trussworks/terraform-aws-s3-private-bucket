...
func TestTerraformAwsS3NoPublicAccessBlock(t *testing.T) {
	t.Parallel()

	tempTestFolder := test_structure.CopyTerraformFolderToTemp(t, "../", "examples/no-public-access-block")

	// Give this S3 Bucket a unique ID for a name tag so we can distinguish it from any other Buckets provisioned
	// in your AWS account
	testName := fmt.Sprintf("terratest-aws-s3-private-bucket-no-public-access-block-%s", strings.ToLower(random.UniqueId()))
	awsRegion := "us-west-2"
	...
	AssertS3BucketPublicAccessBlockConfigurationDisabled(t, terraformOptions)
}
